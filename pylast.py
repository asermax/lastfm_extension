...
class Track(_BaseObject, _Taggable):
    ...
    def get_playcount(self, user=False):
        """Returns the play count."""
        params = None
        
        if user:
            params = self._get_params()
            params['username'] = self.network.get_authenticated_user().get_name()

        doc = self._request("track.getInfo", True, params)
        
        if user:
            res = _number(_extract(doc, 'userplaycount' ))
        else:
            res = _number(_extract(doc, "playcount"))
    ...
    def is_loved(self):
        ...
        return _boolean(_extract(doc, 'userloved' ))
